...
# If there's a build error regression, emit a focused build-failure comment and exit
BUILD_ERROR_MSG=$(jq -r '.regressions[]? | select(.id=="BUILD_ERROR") | .error' "$ANALYSIS" 2>/dev/null || echo "")
if [[ -n "$BUILD_ERROR_MSG" ]]; then
    cat <<EOF
## ❌ Build Failure

**Status:** Build failed during test run.

**Error:** ${BUILD_ERROR_MSG}

<details>
<summary>🧾 View last_output.txt (truncated)</summary>

\
\`\`\`\n$(if [[ -f .test_runs/last_output.txt ]]; then tail -n 300 .test_runs/last_output.txt || true; else echo "No .test_runs/last_output.txt available."; fi)\n\`\`\`
