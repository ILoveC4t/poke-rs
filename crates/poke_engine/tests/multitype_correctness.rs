...
use poke_engine::damage::{
    calculate_damage,
    generations::{Gen4, Gen5, Gen6, Gen7},
};
...
fn test_arceus_judgment_stab_gen4() {
    ...
    assert!(
        result.rolls[0] >= 285 && result.rolls[0] <= 295,
        "Gen4: First damage roll should be ~290 with STAB, got {}",
        result.rolls[0]
    );
    assert!(
        result.rolls[15] >= 340 && result.rolls[15] <= 350,
        "Gen4: Last damage roll should be ~344 with STAB, got {}",
        result.rolls[15]
    );
}
...
fn test_arceus_judgment_stab_gen5() {
    ...
    assert!(
        result.rolls[0] >= 285 && result.rolls[0] <= 295,
        "Gen5: First damage roll should be ~290 with STAB, got {}",
        result.rolls[0]
    );
}
...
fn test_arceus_judgment_stab_gen6() {
    ...
    assert!(
        result.rolls[0] >= 285 && result.rolls[0] <= 295,
        "Gen6: First damage roll should be ~290 with STAB, got {}",
        result.rolls[0]
    );
}
...
fn test_arceus_judgment_stab_gen7() {
    ...
    assert!(
        result.rolls[0] >= 285 && result.rolls[0] <= 295,
        "Gen7: First damage roll should be ~290 with STAB, got {}",
        result.rolls[0]
    );
}
