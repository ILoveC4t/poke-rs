...
pub fn execute(args: AnalyzeArgs) {
    ...
    // Load Head
    let head_summary = load_run(&runs_dir, &args.head)
        .expect("Failed to load head run");
    ...
    let base_summary = load_run(&runs_dir, &base_name)
        .expect("Failed to load base run");

    println!("\n=== Test Regression Analysis ===");
    println!("Comparing:");
    println!("  Base: {} ({})", base_summary.timestamp, base_summary.timestamp_human);
    println!("  Head: {} ({})", head_summary.timestamp, head_summary.timestamp_human);
    println!("================================");
    ...
    let regressions: Vec<&FailureInfo> = head_summary.failures.iter()
        .filter(|f| !base_failures.contains(&f.id))
        .collect();
    
    let fixed: Vec<&FailureInfo> = base_summary.failures.iter()
